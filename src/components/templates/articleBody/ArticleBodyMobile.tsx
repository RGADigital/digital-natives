...
export default function ArticleBodyMobile({ title, chapterTitle, menu, content }: ArticleBodyProps) {
  return (
      ...
      <div className="dn-article-body__content">
        {menu?.length > 0 && (
          <Accordion defaultValue={menu?.[0]?.key} chevron={<Chevron />}>
            {menu.map(item => {
              const { label, href, sectionName = '', key = '' } = item || {}
              if (!label || !href) return null

              return (
                <Accordion.Item key={href} value={key}>
                  <Accordion.Control>
                    <MicroDetail text={label} isCircle />
                  </Accordion.Control>
